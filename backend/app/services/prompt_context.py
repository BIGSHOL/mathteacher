...
### [표기 및 해설 규칙]
1. **빈칸 표기**: 빈칸/네모 박스는 반드시 '□' (특수문자)를 사용하세요. (예: 1 + 2 = □)
2. **세로 계산식**: 세로셈은 반드시 지문 다음 줄에 배치하고, 숫자와 연산자, 구분선(---)을 사용하여 정렬하세요.
3. **상세 해설(Step-by-Step)**: 
   - 덧셈/뺄셈 등 받아올림(Carrying)이나 받아내림(Borrowing)이 있는 연산 문항의 해설은 반드시 자릿수별(일의 자리, 십의 자리...)로 상세히 설명하세요.
   - 예: "일의 자리: 5+6=11이므로 1을 쓰고 십의 자리로 1을 받아올림합니다."
"""
